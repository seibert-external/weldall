...
import type { ReplayStore, VerifiedDpop } from "./types.js";
...
export function normalizeHtu(input: string): string {
  ...
  if (
    ...
    throw new WeldallAuthError("invalid_dpop_proof", "invalid htu scheme");
  if (url.username || url.password)
    throw new WeldallAuthError("invalid_dpop_proof", "htu must not contain credentials");
  url.search = "";
  url.hash = "";
...
export async function verifyStrictDpop(
  ...
  try {
    ...
    throw new WeldallAuthError("invalid_dpop_proof", "invalid DPoP header");
  }
  let payload;
  try {
    ({ payload } = await jwtVerify(proof, await importJWK(proofJwk, "ES256"), {
      algorithms: ["ES256"],
      typ: "dpop+jwt",
    }));
  } catch {
    throw new WeldallAuthError("invalid_dpop_proof", "invalid DPoP signature");
  }
  const { htm, htu, iat, jti, ath } = payload;
  const now = input.now ?? Math.floor(Date.now() / 1000);
  let proofUrl: URL | undefined;
  try {
    if (typeof htu === "string") proofUrl = new URL(htu);
  } catch {
    throw new WeldallAuthError("invalid_dpop_proof", "invalid DPoP htu");
  }
  if (
    typeof htm !== "string" ||
    htm !== input.method.toUpperCase() ||
    ...
    throw new WeldallAuthError("invalid_dpop_proof", "invalid DPoP claims");
  const jkt = await calculateJwkThumbprint(header.jwk, "sha256");
  if (input.expectedJkt !== undefined && !safeEqual(jkt, input.expectedJkt))
    throw new WeldallAuthError("invalid_dpop_proof", "DPoP key mismatch");
  if (input.accessToken) {
    if (typeof ath !== "string" || !safeEqual(ath, base64urlSha256(input.accessToken)))
      throw new WeldallAuthError("invalid_dpop_proof", "access token hash mismatch");
  } else if (ath !== undefined)
    throw new WeldallAuthError("invalid_dpop_proof", "ath is not allowed at this endpoint");
  await consumeReplay(
    input.replay,
    "dpop",
    `${jkt}:${jti}`,
    new Date(((iat as number) + DPOP_MAX_AGE_SECONDS + 1) * 1000),
