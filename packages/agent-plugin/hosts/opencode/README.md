...
- opencode, tested against 1.18
- The Weldall CLI at **0.14.0 or newer**, on the PATH of the shell opencode runs commands in,
  and signed in with `weldall login`
...
opencode has no manifest field for a package's skills, so `plugin.js` puts this package's
skill directory on `config.skills.paths` at startup. That is the whole plugin. It registers no
tools, no hooks of its own, and no MCP server.
