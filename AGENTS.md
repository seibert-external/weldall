...
- `scripts/test-e2e.sh` derives a stable compose project name from the checkout path: compose otherwise derives it from the checkout directory name, so concurrent worktrees named `weldall` share one stack and volumes, and either run's `down --volumes` destroys the other's. Set `COMPOSE_PROJECT_NAME` for an explicit project identity.
