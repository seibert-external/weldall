...
The published CLI requires:

- macOS (credentials and device keys use macOS Keychain)
- Node.js 22.15.0 or newer
- a reachable Weldall HTTPS issuer supplied by your administrator
...
- **Workload client** — a registered machine identity that uses RFC 7523 `private_key_jwt` and DPoP to obtain five-minute Weldall-issued access tokens when the resource and scopes are independently selected and the resource supports every requested scope. See [Workload authentication](apps/docs/src/content/docs/en/workload-authentication.mdx).
