...
/** PostgreSQL-backed replay store shared by every Weldall process. */
export const postgresReplayStore: ReplayStore = {
  async consume(key, expiresAt) {
    ...
    if (Date.now() - lastCleanup > 60_000) {
      ...
      if (expired.length)
        await db.replayMarker.deleteMany({ where: { key: { in: expired.map(({ key }) => key) } } });
    }
