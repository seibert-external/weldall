...
async function post(request: Request) {
  try {
    const { userCode } = await parsePendingRequestBody(request, false);
    ...
    return Response.json(await lookupPendingBrowserConnection(request, user, userCode), {
      headers: { "cache-control": "no-store", pragma: "no-cache" },
    });
  } catch (error) {
    await auditBrowserConnectionFailure(request, "lookup", error);
    return loggedOauthErrorResponse(error);
  }
}

export const POST = withRequestLogging("/api/me/browser-connections/pending/lookup", post);
