...
Weitere unternehmensspezifische Hinweise können Administratoren zentral im [CLI-Appendix](../features/#cli-appendix) hinterlegen. Wenn der Agent `weldall` ohne Unterbefehl aufruft, zeigt die Hilfe das lokal zwischengespeicherte Appendix ohne Netzwerkanfrage an. Anmeldung, Status-, Scope- und Skill-Befehle aktualisieren den Cache für spätere Aufrufe.
...
Der Agent durchsucht zuerst den lokal zwischengespeicherten Katalog mit konkreten Begriffen und Aktionen aus der Aufgabe:

```sh
weldall skills find "Vertrag Prüfdatum"
```

Die Suche berücksichtigt Skill-IDs, Titel, Katalogvorschauen, Tags, Verantwortliche und Ressourcen. Bei mehreren Suchwörtern stehen Skills mit mehr Treffern weiter oben. Eine mögliche Ausgabe ist:

```text
contracts.manage  Verträge verwalten  available
```

Gibt es keinen Treffer, sollte der Agent weniger oder allgemeinere Wörter wie das System, die Ressource oder die Aktion verwenden. `weldall skills` ist die Kurzform von `weldall skills list` und zeigt den vollständigen sichtbaren Katalog. Ein sichtbarer Skill kann je nach Konfiguration verfügbar sein oder auf fehlende Berechtigungen hinweisen.
