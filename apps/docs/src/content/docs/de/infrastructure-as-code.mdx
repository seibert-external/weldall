...
`encryptionKeys` und `connectors` folgen denselben Plan-/Apply-, Import-, State-Move- und Unmanage-Regeln wie andere Objekte. Die vollständige Referenz für nicht geheime Schlüssel- und Connector-Felder steht in der [englischen IaC-Dokumentation](../infrastructure-as-code/#managed-connector-configuration); Einrichtung, unterstützte Vorgänge und Wiederherstellung beschreibt [Managed Google connections](../managed-connectors/).
