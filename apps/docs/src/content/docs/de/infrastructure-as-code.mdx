...
description: Weldall-Einstellungen, Zugriffsobjekte und Connectors mit YAML verwalten.
sidebar:
...
Diese Variablen bestimmen auch, was die Hilfe zeigt: Die IaC-Befehle erscheinen nur dann in `weldall --help`, wenn sie gesetzt sind – eine reine Browser-Anmeldung macht sie nicht nutzbar.
...
Die eingebundenen Dateien sind Fragmente. Sie wiederholen weder `apiVersion` noch `workspace`, sondern enthalten nur eine oder mehrere Objektgruppen wie `scopes`, `resources`, `emailAssignments` oder `connectors`. Die CLI führt alle Fragmente zu einer vollständigen Konfiguration zusammen. Wie die Dateien aufgeteilt werden, kann das Team selbst entscheiden – zum Beispiel nach Anwendung, Zuständigkeit oder Umgebung.
...
`meta.appearance.icon` akzeptiert kanonische Namen in Kebab-Case aus dem [Lucide-Icon-Katalog](https://lucide.dev/icons/). Gradient-Overrides werden nur verwendet, wenn beide Farben eines hellen oder dunklen Paars sechsstellige Hex-Farben sind; andernfalls verwendet Weldall den deterministischen Slug-basierten Fallback.
...
## Managed-Connector-Konfiguration

`connectors` folgen denselben Plan-/Apply-, Import-, State-Move- und Unmanage-Regeln wie andere Objekte. Der Envelope-Provider muss explizit als `envelopeProvider: LOCAL_ENV` angegeben werden und ist nach Erstellung unveränderlich. `requiredScopes` legt fest, wer Verbindungen erstellen und verwenden darf: Eine leere Liste erlaubt allen login-berechtigten Personen den Zugriff, andernfalls müssen alle aufgeführten Weldall-Scopes zugewiesen sein. Alle lokalen Connectoren verwenden `WELDALL_CONNECTOR_KEK`, aber pro Schreibvorgang einen neuen DEK. OAuth-Client-Secrets bleiben ausschließlich schreibbar und werden direkt mit `WELDALL_CREDENTIAL_ENCRYPTION_KEY` verschlüsselt. OpenBao, unbekannte Provider-Konfigurationsfelder und beliebige ENV-Schlüsselnamen werden nicht akzeptiert. Die vollständige Referenz steht in der [englischen IaC-Dokumentation](../infrastructure-as-code/#managed-connector-configuration); Einrichtung und Wiederherstellung beschreibt [Managed Google connections](../managed-connectors/).
