...
<Aside type="tip" title="Warum IaC?">
  In einem versionierten Git-Repository bleibt jede Änderung nachvollziehbar. Pull Requests bieten Raum für Diskussionen und Freigaben, und Kommentare im YAML dokumentieren direkt, warum eine bestimmte Berechtigung existiert.
</Aside>

<Aside title="Atomare Änderungen">
  Weldall fasst die YAML-Dateien in einem Workspace zusammen und wendet den gewünschten Zustand atomar an: Entweder werden alle geplanten Änderungen durchgeführt oder keine.
</Aside>
...
<FileTree>
- platform-access/
  - **weldall.yml** Root-Manifest
  - weldall.lock.yml Versionierter Serverzustand und Objektzuordnungen
  - weldall/
    - access.yml Scopes und Zuweisungen
    - runtime.yml Ressourcen und Machine Clients
    - imports/ Optional, wird von `weldall import` verwendet
