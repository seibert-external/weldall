...
2. In **Administration → Connectors & keys**, create an encryption key, register version `1` with the `local-env` provider and that environment variable, then select it as active. Provider configuration and immutable provider state live in PostgreSQL; the actual key does not.
...
`local-env` is currently the only built-in key provider; this release does not claim native AWS KMS, Google Cloud KMS, Azure Key Vault or Vault/OpenBao integration. The provider boundary keeps public source configuration separate from opaque provider state so future providers can store wrapped data keys without changing the encryption call sites.
