...
2. In **Administration → Connectors**, create a Google connector with a stable key such as `google`, its OAuth client ID and client secret, enabled APIs, allowed permissions and explicit default permissions. Select **Local environment key** (`LOCAL_ENV`) as the envelope provider. This choice is immutable; detail/edit views show it read-only. **OpenBao — Upcoming** is disabled and not yet supported by UI, server APIs or IaC.
...
The owner/CLI disconnect first commits **REVOCATION_PENDING**, blocking new requests, then makes one bounded provider call outside the transaction. Confirmed revocation removes encrypted credentials. Failure or interruption retains encrypted retry material but never permits normal use; explicitly repeat disconnect. Even `invalid_token` is not treated as proof that the account/client grant was revoked: an interrupted refresh may have rotated that token. If refresh is in flight, retry disconnect after it finishes so any rotated token can be revoked. Google revocation can affect **other authorizations for the same account/client**. Already-dispatched requests cannot be retroactively cancelled.

Administrators inspect permissions, request dispatch counts, last use, health and revocation state under **Managed connections**. Audit records include actor, connection/account, sanitized operation, outcome and duration, not tokens, request bodies or sensitive query values. Owner deletion requires a confirmed disconnect and no outstanding authorization attempts. Connector removal remains blocked while dependent connections/attempts remain. Completed/expired non-sensitive attempts are cleaned in bounded batches when setup or connector removal runs.

Authorization attempts are internal OAuth state, not an administrative table or deletion API. A failed callback that obtained tokens retains them for owner-initiated cancellation and revocation. An interrupted exchange may have created a grant even if no tokens reached storage; remove such access in Google account settings. Stuck attempts are not automatically discarded and can block connector removal; no administrative attempt-cleanup workflow is currently provided.

The admin overview shows connection/account, owner, connector, status and last use. Search by connection, account or owner and filter by status or connector; filters apply to the 200 most recent connections and are kept in the URL. Open a row for the connection's identity, selected and granted permissions, effective access, usage and timestamps. Each detail page has one red **Disconnect** action. After confirmation it blocks use, attempts Google revocation for the connection and any retained reconnect tokens, and permanently deletes the connection, linked attempts and encrypted data—even if Google or decryption fails. Unconfirmed revocation produces a warning and an audit record; remove access in Google account settings because Weldall can no longer retry. Active refreshes or token exchanges must finish or expire before removal; the connection remains blocked in the meantime. There are no separate Delete or Terminal cleanup actions. This does not change the owner/CLI disconnect-and-delete workflow; reconnect can preserve an owner-disconnected connection's ID until it is deleted.
