...
Administrators inspect permissions, request dispatch counts, last use, health and revocation state under **Managed connections**. Audit records include actor, connection/account, sanitized operation, outcome and duration, not tokens, request bodies or sensitive query values. Connector removal remains blocked while dependent connections/attempts remain. Completed/expired non-sensitive attempts are cleaned in bounded batches when setup or connector removal runs.
